...
function generateNextJsPreviewComponent(components: ComponentInfo[], explorerPort: number): string {
  // Filter to only include client components (those with 'use client' directive)
  // Server components or components with server-only dependencies cannot be dynamically imported in a client component
  const clientComponents = components.filter(comp => comp.isClientComponent);

  log.info(`Filtered ${components.length} components to ${clientComponents.length} client components for preview registry`);

  // Generate import statements for client components only
  const imports = clientComponents.map((comp) => {
...
async function setupNextJsAppPreview(
  ...
  // Create a minimal layout.tsx - just passes through children
  // In Next.js App Router, only the root layout can have <html> and <body> tags
  const layoutContent = `/**
 * Preview Layout - Auto-generated by Storial
 *
 * This minimal layout wraps preview pages without adding extra UI.
 * DO NOT EDIT - This file is managed by Storial.
 */
...
`;
  ...
  const layoutRelativePath = hasSrc ? 'src/app/storial-preview/layout.tsx' : 'app/storial-preview/layout.tsx';
  filesCreated.push(layoutRelativePath);
  log.success('Created Next.js preview layout', { path: layoutPath });

  // Create storial-preview/page.tsx
