...
async function setupReactRouterPreview(
  ...
  try {
    ...
    if (!appContent.includes('__Canvas')) {
      ...
      const routeToAdd = `        <Route path="/__preview" element={<Canvas />} />\n`;
...
async function setupNextJsAppPreview(
  ...
  // Create __preview directory
  const previewDir = path.join(appDir, '__preview');
  await fs.mkdir(previewDir, { recursive: true });
  
  // Create a minimal layout.tsx that doesn't inherit root layout's server components
  // This prevents Navbar/Footer from being rendered (which may call external APIs)
  const layoutContent = `/**
 * Preview Layout - Auto-generated by Explorer
 * 
 * This minimal layout isolates preview pages from the root layout,
 * preventing server components (like Navbar, Footer) from running
 * and making external API calls during component preview.
 * 
 * DO NOT EDIT - This file is managed by the Explorer tool.
 */
...
`;
  ...
  const layoutRelativePath = hasSrc ? 'src/app/__preview/layout.tsx' : 'app/__preview/layout.tsx';
  filesCreated.push(layoutRelativePath);
  log.success('Created Next.js preview layout', { path: layoutPath });
  
  // Create __preview/page.tsx
  ...
  const pageRelativePath = hasSrc ? 'src/app/__preview/page.tsx' : 'app/__preview/page.tsx';
...
async function setupNextJsPagesPreview(
  ...
  const previewPath = path.join(pagesDir, '__preview.tsx');
  const previewContent = generateNextJsPreviewComponent(components, explorerPort);
  
  await fs.writeFile(previewPath, previewContent, 'utf-8');
  
  const relativePath = hasSrc ? 'src/pages/__preview.tsx' : 'pages/__preview.tsx';
...
export async function checkPreviewStatus(
  ...
  try {
    ...
    if (routerType === 'react-router' || routerType === 'unknown') {
      ...
      const hasSrc = await fs.access(path.join(projectPath, 'src', 'app')).then(() => true).catch(() => false);
      previewPath = hasSrc 
        ? path.join(projectPath, 'src', 'app', '__preview', 'page.tsx')
        : path.join(projectPath, 'app', '__preview', 'page.tsx');
    } else {
      const hasSrc = await fs.access(path.join(projectPath, 'src', 'pages')).then(() => true).catch(() => false);
      previewPath = hasSrc
        ? path.join(projectPath, 'src', 'pages', '__preview.tsx')
        : path.join(projectPath, 'pages', '__preview.tsx');
    }
  ...
  try {
    if (routerType === 'react-router' || routerType === 'unknown') {
      ...
      try {
        ...
        appContent = appContent.replace(/\s*<Route path="\/__preview" element={<Canvas \/>} \/>\n/g, '');
        await fs.writeFile(appPath, appContent, 'utf-8');
      } catch {}
      ...
      const previewDir = hasSrc
        ? path.join(projectPath, 'src', 'app', '__preview')
        : path.join(projectPath, 'app', '__preview');
